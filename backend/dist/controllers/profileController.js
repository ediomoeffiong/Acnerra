...
exports.removePartner = exports.addPartner = exports.getPartners = exports.searchProfiles = exports.updateProfile = exports.getProfileByUsername = void 0;
...
const getPartners = async (req, res) => {
    ...
    try {
        const user = await User_1.User.findById(req.user.userId).populate('accountabilityPartners', 'id username name bio image');
        if (!user) {
            return res.status(404).json({ message: "User not found" });
        }
...
const removePartner = async (req, res) => {
    ...
    try {
        ...
        user.accountabilityPartners = (user.accountabilityPartners || []).filter((id) => id.toString() !== partnerId);
        await user.save();
        await user.populate('accountabilityPartners', 'id username name bio image');
        return res.status(200).json({
